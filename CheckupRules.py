...
rc.send('''fw_uid=$(mgmt_cli -r true show package name "Standard" --format json | $CPDIR/jq/jq '."access-layers"[] | select (.name=="FW_Layer") | .uid')''')
time.sleep(10)
rc.send('''mgmt_cli -r true set generic-object uid $fw_uid implicitCleanupDrop "false"''')
time.sleep(15)
rc.send('''auf_uid=$(mgmt_cli -r true show package name "Standard" --format json | $CPDIR/jq/jq '."access-layers"[] | select (.name=="APP&URLF") | .uid')''')
time.sleep(10)
rc.send('''mgmt_cli -r true set generic-object uid $auf_uid implicitCleanupDrop "false"''')
time.sleep(15)
rc.send('''nw_uid=$(mgmt_cli -r true show package name "Standard" --format json | $CPDIR/jq/jq '."access-layers"[] | select (.name=="Network") | .uid')''')
time.sleep(15)
