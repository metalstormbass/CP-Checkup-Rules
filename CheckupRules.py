...
rc.send('''fwl_uid=$(mgmt_cli -r true show package name '''+ policy_name+ ''' --format json | $CPDIR/jq/jq '."access-layers"[] | select (.name=="FW_Layer") | .uid')''')
rc.send("\n")
time.sleep(8)
rc.send('mgmt_cli -r true set generic-object uid $fwl_uid implicitCleanupDrop "false"')
rc.send("\n")
time.sleep(8)
rc.send('''auf_uid=$(mgmt_cli -r true show package name ''' + policy_name + ''' --format json | $CPDIR/jq/jq '."access-layers"[] | select(.name=="APP&URLF") | .uid')''')
rc.send("\n")
time.sleep(8)
rc.send('''mgmt_cli -r true set generic-object uid $auf_uid implicitCleanupDrop "false"''')
rc.send("\n")
time.sleep(8)
rc.send('''nw_uid=$(mgmt_cli -r true show package name ''' + policy_name + ''' --format json | $CPDIR/jq/jq '."access-layers"[] | select (.name=="Network") | .uid')''')
rc.send("\n")
time.sleep(8)
